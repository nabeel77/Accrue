...
use crate::kamino::{read_obligation_loan_to_value_bps, read_reserve_account};
...
pub struct WithdrawCollateral<'info> {
    ...
    /// CHECK: checked against the position's market, refreshed so the loan to value is current
    #[account(mut, owner = KAMINO_LEND_PROGRAM_ID)]
...
pub fn handle_withdraw_collateral(
    ...
    require!(
        collateral_token_amount <= ledger_before.obligation_collateral,
        AccrueError::PositionSizeOutOfRange
    );
    ...
    move_the_stock_to_the_owner(accounts, &position_seeds)?;
    ...
    let loan_to_value_bps =
        read_obligation_loan_to_value_bps(&accounts.obligation.to_account_info())?;
    require!(
        loan_to_value_bps <= accounts.position.strategy.target_ltv_bps,
    ...
    assert_invariants_hold(&InvariantCheck {
        ...
        collateral_movement: CollateralMovement::ExactlyOut(collateral_token_amount),
...
fn move_the_stock_to_the_owner(
    accounts: &WithdrawCollateral<'_>,
    position_seeds: &[&[u8]],
) -> Result<()> {
    let amount = token_account_amount(&accounts.position_collateral_account.to_account_info())?;
    if amount == 0 {
        return Ok(());
    }
    anchor_spl::token_interface::transfer_checked(
        CpiContext::new_with_signer(
            ...
            anchor_spl::token_interface::TransferChecked {
                from: accounts.position_collateral_account.to_account_info(),
                mint: accounts.collateral_mint.to_account_info(),
                to: accounts.owner_collateral_account.to_account_info(),
...
fn refresh_both_reserves(accounts: &WithdrawCollateral<'_>) -> Result<()> {
    refresh_reserve(
        &ReserveRefresh {
            reserve: accounts.collateral_reserve.to_account_info(),
            lending_market: accounts.lending_market.to_account_info(),
