...
import { positionByAddress } from '../../../../server/positions/list.js';
...
/**
 * The position's own USDC account starts and ends near empty, so what was repaid or borrowed is
 * the size of the move on the lending market's side, which is the only other USDC account a guard
 * call touches apart from the caller's bounty.
 */
...
/**
 * Read from the chain, not from our own keeper, so the activity list is true even when somebody
 * else ran the guard.
 */
export async function POST(request: Request): Promise<Response> {
  ...
  if (programId === undefined) {
    return refuse('No program configured.', 409);
  }
